...
public class HttpTemperature {
    ...
    public Temperature getTemperature(String location) {
        logger.debug(MarkerManager.getMarker("SERVER"), "Получение координат для " + location);
        //получаем координаты из строки
        Coordinate coordinate = getCoordinates(location);
        if (coordinate == null) {
            return null;
        } else {
            logger.debug(MarkerManager.getMarker("SERVER"), "Координаты получены " + coordinate);
        }

        logger.debug(MarkerManager.getMarker("SERVER"), "Получение температуры для " + location);
        ...
        try {
            ...
            if (temperature != null) {
                ...
                logger.debug(MarkerManager.getMarker("SERVER"), "Disconnect YandexWeather");
            }
        }
    ...
    private Coordinate getCoordinates(String request) {
        ...
        try {
            ...
            logger.debug(MarkerManager.getMarker("SERVER"), "Disconnect geocoordinating");
        }
        return null;
    }
